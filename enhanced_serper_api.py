...
class EnhancedSerperAPI:
    ...
    def _is_valid_vendor_result(self, result: Dict, category: str) -> bool:
        """Relaxed validation to allow more legitimate vendor results"""
        title = result.get('title', '').lower()
        snippet = result.get('snippet', '').lower()
        link = result.get('link', '').lower()

        # Only exclude obvious non-vendor content
        exclude_patterns = [
            'blog', 'article', 'guide', 'tips', 'how to',
            'wikipedia', 'wiki', 'news', 'review site'
        ]

        for pattern in exclude_patterns:
            if pattern in title or pattern in link:
                return False

        # Accept results from known business directories automatically
        business_domains = [
            'justdial.com', 'sulekha.com', 'urbanpro.com', 'weddingz.in',
            'indiamart.com', 'yellowpages.in', 'asklaila.com'
        ]
        ...
        # Look for any business indicators (more permissive)
        business_indicators = [
            'contact', 'phone', 'booking', 'enquiry', 'call', 'service',
            'professional', 'studio', 'company', 'vendor', 'hall', 'venue'
        ]

        has_business_indicator = any(indicator in snippet or indicator in title 
                                   for indicator in business_indicators)

        # Accept if has business indicators OR if title suggests it's a specific business
        is_specific_business = len(title.split()) <= 8 and not any(word in title for word in ['top', 'best', 'list'])

        return has_business_indicator or is_specific_business
